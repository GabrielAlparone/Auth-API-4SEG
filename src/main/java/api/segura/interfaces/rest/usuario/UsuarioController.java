...
import api.segura.domain.usuario.Usuario;
import api.segura.domain.usuario.UsuarioDomainService;
import api.segura.application.usuario.TentativaLoginService;
import api.segura.application.usuario.CodigoVerificacaoService;
import api.segura.infrastructure.email.EmailService;
import api.segura.util.InputSanitizer;
import api.segura.util.jwtUtil;
...
public class UsuarioController {
    ...
    public ResponseEntity<String> registrar(@RequestBody Usuario usuario, HttpServletRequest request) {
        ...
        UsuarioDomainService.registrar(usuario);
    ...
    public ResponseEntity<String> login(@RequestBody LoginRequest loginRequest, HttpServletRequest request) {
        ...
        Optional<Usuario> usuarioOpt = UsuarioDomainService.buscarPorUsername(username);
        ...
        if (!UsuarioDomainService.verificarSenha(senha, usuario.getSenha())) {
    ...
    public ResponseEntity<String> verificarCodigo(@RequestBody CodigoVerificacaoRequest request) {
        ...
        if (!valido) {
            System.out.println(codigoVerificacaoService.validarCodigo(username, codigo));
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body("Código inválido ou expirado.");
        }
         String token = jwtUtil.generateToken(username);
         return ResponseEntity.ok("Código válidado, token do usuário: " + token);
